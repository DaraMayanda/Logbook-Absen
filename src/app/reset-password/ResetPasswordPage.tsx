...
export default function ResetPasswordPage() {
  ...
  // token dari query (supabase login sementara)
  const accessToken = searchParams.get('access_token')

  // cek apakah user login sementara dari link reset password
  useEffect(() => {
    const checkUser = async () => {
      const { data: { user }, error } = await supabase.auth.getUser()
      if (!user || error) setError('Link reset password tidak valid atau sudah kadaluarsa.')
  ...
  const handleReset = async (e: React.FormEvent) => {
    ...
    try {
      // update password user (Supabase v3+)
